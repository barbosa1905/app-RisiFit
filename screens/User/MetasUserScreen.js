// screens/User/MetasUserScreen.js
import React, { useEffect, useState, useCallback } from 'react';
import {
  View, Text, StyleSheet, TouchableOpacity, StatusBar, Platform,
  ScrollView, TextInput, Alert
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ionicons } from '@expo/vector-icons';
import { LinearGradient } from 'expo-linear-gradient';
import { useNavigation } from '@react-navigation/native';
import { auth, db } from '../../services/firebaseConfig';
import { doc, getDoc, updateDoc, setDoc } from 'firebase/firestore';
import Colors from '../../constants/Colors';

export default function MetasUserScreen() {
  const navigation = useNavigation();
  const user = auth.currentUser;

  const [loading, setLoading] = useState(true);
  const [weeklyMinutes, setWeeklyMinutes] = useState('150'); // OMS 🙂
  const [weightTarget, setWeightTarget] = useState('');
  const [stepsPerDay, setStepsPerDay] = useState('8000');

  const load = useCallback(async () => {
    if (!user?.uid) return setLoading(false);
    try {
      const snap = await getDoc(doc(db, 'users', user.uid));
      if (snap.exists()) {
        const g = snap.data()?.goals || {};
        if (g.weeklyMinutes) setWeeklyMinutes(String(g.weeklyMinutes));
        if (g.weightTarget) setWeightTarget(String(g.weightTarget));
        if (g.stepsPerDay) setStepsPerDay(String(g.stepsPerDay));
      }
    } catch (e) {
      console.error(e);
      Alert.alert('Erro', 'Não foi possível carregar as tuas metas.');
    } finally {
      setLoading(false);
    }
  }, [user?.uid]);

  useEffect(() => { load(); }, [load]);

  const save = async () => {
    try {
      setLoading(true);
      const ref = doc(db, 'users', user.uid);
      const goals = {
        weeklyMinutes: Number(weeklyMinutes) || 0,
        weightTarget: Number(weightTarget) || 0,
        stepsPerDay: Number(stepsPerDay) || 0,
        updatedAt: new Date(),
      };
      const snap = await getDoc(ref);
      if (snap.exists()) await updateDoc(ref, { goals });
      else await setDoc(ref, { goals }, { merge: true });
      Alert.alert('Sucesso', 'Metas guardadas.');
      navigation.goBack();
    } catch (e) {
      console.error(e);
      Alert.alert('Erro', 'Não foi possível guardar as metas.');
    } finally {
      setLoading(false);
    }
  };

  return (
    <SafeAreaView style={styles.safe}>
      <StatusBar barStyle="light-content" backgroundColor={Colors.primary} />
      {/* Top Bar */}
      <View style={styles.topBarWrap}>
        <LinearGradient colors={[Colors.primary, '#22313B']} style={StyleSheet.absoluteFill} />
        <View style={styles.topBarRow}>
          <TouchableOpacity onPress={() => navigation.goBack()} style={styles.iconBtn}>
            <Ionicons name="arrow-back" size={20} color={Colors.onPrimary} />
          </TouchableOpacity>
          <View style={styles.brandCenter} pointerEvents="none">
            <Text style={styles.brandRisi}>RISI</Text>
            <Text style={styles.brandFit}> FIT</Text>
          </View>
          <View style={{ width: 36 }} />
        </View>
      </View>

      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Text style={styles.title}>Metas</Text>
        <Text style={styles.subtitle}>Define objetivos claros para manteres a consistência.</Text>

        <Card>
          <Field label="Minutos por semana">
            <TextInput
              style={styles.input}
              keyboardType="numeric"
              value={weeklyMinutes}
              onChangeText={setWeeklyMinutes}
              placeholder="Ex.: 150"
            />
          </Field>

          <Field label="Peso objetivo (kg)">
            <TextInput
              style={styles.input}
              keyboardType="numeric"
              value={weightTarget}
              onChangeText={setWeightTarget}
              placeholder="Ex.: 78"
            />
          </Field>

          <Field label="Passos por dia">
            <TextInput
              style={styles.input}
              keyboardType="numeric"
              value={stepsPerDay}
              onChangeText={setStepsPerDay}
              placeholder="Ex.: 8000"
            />
          </Field>
        </Card>

        <TouchableOpacity disabled={loading} onPress={save} style={styles.saveBtn}>
          <Text style={styles.saveText}>{loading ? 'A guardar…' : 'Guardar metas'}</Text>
          <Ionicons name="checkmark" size={18} color={Colors.onPrimary} />
        </TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
}

function Card({ children }) {
  return (
    <View style={styles.card}>
      {children}
    </View>
  );
}

function Field({ label, children }) {
  return (
    <View style={{ marginBottom: 12 }}>
      <Text style={styles.fieldLabel}>{label}</Text>
      {children}
    </View>
  );
}

const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: Colors.background },
  topBarWrap: {
    backgroundColor: Colors.primary,
    paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight || 0) + 6 : 10,
    paddingBottom: 14, borderBottomLeftRadius: 24, borderBottomRightRadius: 24, overflow: 'hidden',
  },
  topBarRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, justifyContent: 'space-between' },
  brandCenter: { flexDirection: 'row', alignItems: 'center' },
  brandRisi: { color: Colors.onPrimary, fontWeight: '900', fontSize: 18, letterSpacing: 1 },
  brandFit: { color: Colors.secondary, fontWeight: '900', fontSize: 18, letterSpacing: 1 },
  iconBtn: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,0.12)' },

  title: { fontSize: 20, fontWeight: '900', color: Colors.textPrimary, marginBottom: 4 },
  subtitle: { color: Colors.textSecondary, marginBottom: 12 },

  card: { backgroundColor: Colors.surface, borderRadius: 16, padding: 16, borderWidth: 1, borderColor: Colors.divider, marginBottom: 14 },
  fieldLabel: { color: Colors.textSecondary, marginBottom: 6, fontWeight: '700' },
  input: {
    backgroundColor: Colors.surface, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12,
    borderWidth: 1, borderColor: Colors.divider, color: Colors.textPrimary,
  },

  saveBtn: {
    marginTop: 6, backgroundColor: Colors.primary, paddingVertical: 14, borderRadius: 14,
    alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8,
  },
  saveText: { color: Colors.onPrimary, fontWeight: '900', marginRight: 6 },
});
